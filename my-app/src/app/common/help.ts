...
import {IntersectingRectangle, Point} from './utils';

export class Help {
  private static cosTable: number[] = [
    1.0,
    0.9997,
    0.9988,
    0.99729,
  ...
  static getCursorPosition(ev: JQueryEventObject): Point {
    if (
      ev.originalEvent &&
      (ev.originalEvent as any).targetTouches &&
      (ev.originalEvent as any).targetTouches.length > 0
    ) {
      ev = (ev.originalEvent as any).targetTouches[0] as any;
    }
  ...
  static safeResize(block: CanvasInformation, width: number, height: number): CanvasInformation {
    const m = CanvasInformation.create(width, height, false);
  ...
  static getQueryString(): {[key: string]: string} {
    ...
    while ((m = re.exec(queryString)) != null) {
      result[(window as any).decodeURIComponent(m[1])] = (window as any).decodeURIComponent(m[2]);
    }
    return result;
  }
